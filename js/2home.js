 ...
 *  - bindLevelToggle(): подтверждение → сброс прогресса текущего словаря → запись режима → мягкая перерисовка
...
(function () {
  ...
  function tConfirm() {
    ...
    return uk
      ? {
          title: 'Змінити режим?',
          text: 'Перемикання режиму очистить прогрес цього словника (зірочки буде скинуто). Продовжити?',
          cancel: 'Скасувати',
          ok: 'Продовжити'
        }
      : {
          title: 'Сменить режим?',
          text: 'Переключение режима очистит прогресс этого словаря (звёзды будут сброшены). Продолжить?',
          cancel: 'Отмена',
          ok: 'Продолжить'
        };
  }
  ...
  // Мини-модалка подтверждения (самодостаточная, без зависимостей)
  function miniConfirm(opts){
    const { title, text, okText, cancelText } = opts || {};
    return new Promise((resolve) => {
      const wrap = document.createElement('div');
      wrap.className = 'mmodal-wrap';
      const css = `
.mmodal-wrap{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:2147483646;background:rgba(0,0,0,.35);backdrop-filter:blur(2px)}
.mmodal{max-width:min(92vw,480px);margin:6vw;background:var(--mm-bg,#131a24);color:var(--mm-fg,#fff);border-radius:16px;border:1px solid rgba(255,255,255,.12);box-shadow:0 10px 30px rgba(0,0,0,.25);padding:16px 16px 12px;font:14px/1.45 system-ui,-apple-system,Segoe UI,Roboto,Arial}
.mmodal h3{margin:0 0 6px;font:600 16px/1.3 system-ui,-apple-system,Segoe UI,Roboto,Arial}
.mmodal p{margin:0 0 12px;opacity:.9}
.mmodal .btns{display:flex;gap:8px;justify-content:flex-end}
.mmodal button{cursor:pointer;border:1px solid rgba(255,255,255,.35);background:transparent;color:inherit;border-radius:12px;padding:.5rem .8rem}
.mmodal button.primary{background:#fff;color:#111;border-color:#fff}
@media (prefers-color-scheme: light){
  .mmodal{--mm-bg:#fff;--mm-fg:#111;border-color:#e5e8ef}
  .mmodal button{border-color:#c7cfdb}
  .mmodal button.primary{background:#111;color:#fff;border-color:#111}
}`;
      const style = document.createElement('style'); style.textContent = css;
      const box = document.createElement('div'); box.className = 'mmodal';
      box.innerHTML = `
        <h3>${title || ''}</h3>
        <p>${text || ''}</p>
        <div class="btns">
          <button class="btn-cancel">${cancelText || 'Cancel'}</button>
          <button class="primary btn-ok">${okText || 'OK'}</button>
        </div>`;
      wrap.appendChild(style); wrap.appendChild(box);
      document.body.appendChild(wrap);

      const cleanup = (v)=>{ try{wrap.remove();}catch(_){} resolve(v); };
      wrap.addEventListener('click', (e)=>{ if (e.target === wrap) cleanup(false); });
      box.querySelector('.btn-cancel').onclick = ()=> cleanup(false);
      box.querySelector('.btn-ok').onclick = ()=> cleanup(true);
    });
  }
  ...
  function resetCurrentDeckProgress(key){
    try {
      ...
      // Mistakes-колоды: если активен спец. ключ — пытаемся очистить через API, иначе звёзды уже очищены выше
      try {
  ...
  function bindLevelToggle() {
    ...
    t.addEventListener('change', async () => {
      ...
      const needConfirm = hasProgressInDeck(key); // как в «старой»: если есть прогресс — спросим подтверждение
      ...
      if (needConfirm) {
        ...
        if (!ok) { // откат тумблера
          t.checked = (before === 'hard');
          return;
        }
        // очищаем прогресс активного словаря
