/* ==========================================================
 * home.js — Главная (двухфазная отрисовка звёзд + режим per-deck)
 *  - setUiLang(): dataset.lang + html@lang + save + event
 *  - bindLevelToggle(): подтверждение → сброс прогресса ТЕКУЩЕГО СЕТА → запись режима per-deck → мягкая перерисовка
 *  - getModeForKey()/setModeForKey(): режим сложности хранится по каждому словарю отдельно
...
(function () {
  ...
  /* ------------------------ Режим per-deck (normal/hard) ------------------------ */
  function getModeForKey(key) {
    try {
      const map = (A.settings && A.settings.levelByDeck) || null;
      if (map && typeof map[key] === 'string') {
        return map[key] === 'hard' ? 'hard' : 'normal';
      }
    } catch(_) {}
    // Фолбэк: глобальная настройка/атрибут (для обратной совместимости)
    const g = (A.settings && (A.settings.level || A.settings.mode)) || (document.documentElement.dataset.level || 'normal');
    return String(g).toLowerCase() === 'hard' ? 'hard' : 'normal';
  }
  function setModeForKey(key, mode) {
    A.settings = A.settings || {};
    A.settings.levelByDeck = A.settings.levelByDeck || {};
    A.settings.levelByDeck[key] = (mode === 'hard' ? 'hard' : 'normal');
    try { A.saveSettings && A.saveSettings(A.settings); } catch(_) {}
  }

  // Экспортируем совместимые хелперы (под активный словарь)
  if (typeof A.getMode !== 'function') {
    A.getMode = function(){ return getModeForKey(activeDeckKey()); };
  }
  if (typeof A.getStarStep !== 'function') {
    A.getStarStep = function(){ return (A.getMode() === 'normal') ? 1 : 0.5; };
  }
  ...
  function syncLevelUi(){
    ...
    if (t) t.checked = (mode === 'hard');
  }

  // Переключатель сложности с очисткой прогресса ТЕКУЩЕГО СЕТА
  function bindLevelToggle() {
    const t = document.getElementById('levelToggle');
    if (!t) return;

    t.checked = (A.getMode() === 'hard'); // checked => hard

    t.addEventListener('change', () => {
      const key    = activeDeckKey();
      const before = getModeForKey(key);
      const want   = t.checked ? 'hard' : 'normal';
      if (before === want) return;

      // Проверяем прогресс только в ТЕКУЩЕМ СЕТЕ
      ...
      // Если есть прогресс — подтверждение и при согласии чистим только текущий сет
      if (hasProgress) {
        const uk  = (getUiLang() === 'uk');
        const msg = uk
          ? 'Перемикання режиму очистить прогрес поточного набору. Продовжити?'
          : 'Переключение режима очистит прогресс текущего набора. Продолжить?';
      ...
      // Записываем режим именно для текущего словаря
      setModeForKey(key, want);
      syncLevelUi();
  ...
  function getCurrentSliceWordIds(key){
    // Сначала пробуем фактический слайс тренера
    try {
  ...
  function renderSets() {
    ...
    // Синхронизируем UI режима под активный сет/словарь
    syncLevelUi();
  }
  ...
  function mountApp() {
    ...
    syncLevelUi();              // ← режим для активного словаря
