...
(function () {
  ...
  function safeDeckKey(prefer){
    // 1) если Trainer уже держит валидный ключ — используем его
    try {
    ...
    // 2) если есть prefer и он валиден — берём его
    if (isValidDeckKey(prefer)) return prefer;

    // 3) иначе пробуем lastDeckKey из настроек
    try {
      const k = (A.settings && A.settings.lastDeckKey) || null;
      if (isValidDeckKey(k)) return k;
    } catch(_){}

    // 4) иначе — первый доступный базовый словарь
  ...
  function activeDeckKey() {
    try {
      // предпочитаем вернуться туда, откуда пользователь зашёл в МО/Избранное
      const prefer = (A.settings && A.settings.preferredReturnKey) || null;
      const key = safeDeckKey(prefer);

      // синхронизируем Trainer и lastDeckKey, чтобы Home точно показала сет
  ...
  const Router = {
    ...
    routeTo(action) {
      ...
      if (action === 'mistakes') {
        // запомним откуда пришли — при удалении виртуальной деки вернёмся сюда
        try {
          A.settings = A.settings || {};
          A.settings.preferredReturnKey = activeDeckKey();
          A.saveSettings && A.saveSettings(A.settings);
        } catch(_){}
