 ...
 *  - bindLevelToggle(): подтверждение → сброс прогресса ТЕКУЩЕГО СЕТА → запись режима → мягкая перерисовка
...
(function () {
  ...
  // ✅ Переключатель сложности с очисткой ТЕКУЩЕГО СЕТА
  function bindLevelToggle() {
    ...
    t.addEventListener('change', () => {
      ...
      // Проверяем прогресс в ТЕКУЩЕМ СЕТЕ
      ...
      try {
        ...
        for (let i = 0; i < ids.length; i++) {
          const v = Number(st[starKey(ids[i], key)] || 0);
  ...
  function getCurrentSliceWordIds(key){
    // Сначала пробуем фактический слайс тренера (если он даёт текущую выборку)
    try {
